...
pub fn link_sessions(child_index: usize, parent_index: usize) -> Result<()> {
    ...
    if metadata
        .values()
        .any(|m| m.parent_session_id.as_ref() == Some(&child.id))
    {
        anyhow::bail!(
            "Session [{}] is already a parent. Cannot link it as a child.\nCompaction creates linear chains: each session can only be a child of one parent.",
            child_index
        );
    }
    ...
    if metadata
        .values()
        .any(|m| m.parent_session_id.as_ref() == Some(&parent.id))
    {
        anyhow::bail!(
            "Session [{}] already has a child. Cannot link another child to it.\nCompaction creates linear chains: each session can only have one child.",
            parent_index
        );
    }
    ...
    if let Some(existing) = metadata.get(&child.id) {
        if existing.parent_session_id.is_some() {
            anyhow::bail!(
                "Session [{}] is already linked to a parent. Use 'ksm unlink {}' first if you want to change the parent.",
                child_index,
                child_index
            );
        }
